...
const Contactos = () => {
  ...
  const handleEdit = (item) => {
    ...
    setFormData({
      ...
      obserbacion: item.obserbacion || '',
      ingerencia: item.ingerencia || ''
    });
    setIsDialogOpen(true);
  };
  ...
  const handleDelete = async (item) => {
    if (window.confirm('¿Está seguro de eliminar este contacto?')) {
      try {
        await contactosService.delete(item.id);
        await loadData();
      } catch (err) {
        setError(err.message);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedItem) {
        await contactosService.update(selectedItem.id, formData);
      } else {
  ...
  const columns = [
    ...
    { key: 'ingerencia', label: 'Ingerencia' },
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="bg-white shadow-sm rounded-md p-4 sm:p-6">
        <h1 className="text-2xl font-semibold mb-2">Contactos</h1>
        <p className="text-gray-500 mb-4">Gestiona la información de contactos</p>
      ...
      <FormDialog
        ...
        title={selectedItem ? 'Editar Contacto' : 'Nuevo Contacto'}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="grid w-full gap-2">
            <label htmlFor="telefono2" className="text-sm font-medium text-gray-700">
              Teléfono Secundario
            </label>
            <Input
              id="telefono2"
              ...
              onChange={(e) =>
                setFormData({ ...formData, telefono2: e.target.value })
              }
            />
          </div>
          ...
          <div className="grid w-full gap-2">
            <label htmlFor="ingerencia" className="text-sm font-medium text-gray-700">
              Ingerencia
            </label>
            <Input
              id="ingerencia"
              ...
              onChange={(e) =>
                setFormData({ ...formData, ingerencia: e.target.value })
              }
            />
          </div>
          ...
          <div className="grid w-full gap-2 md:col-span-2">
            <label htmlFor="obserbacion" className="text-sm font-medium text-gray-700">
              Observación
            </label>
            <Textarea
              id="obserbacion"
              ...
              onChange={(e) =>
                setFormData({ ...formData, obserbacion: e.target.value })
              }
              rows={3}
            />
          </div>
