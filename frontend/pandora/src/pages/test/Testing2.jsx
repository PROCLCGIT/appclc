...
// ────────────────────────────────────────────────────────────────────────────────
// VENTAS SERVICE UNIFICADO PARA RETORNAR SOLO 'data'
// ────────────────────────────────────────────────────────────────────────────────
const ventasService = {
  getAll: async (params) => {
    try {
...
// Usamos el servicio de productos disponibles, adaptándolo al formato que espera la página
...
const Vent = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    valor: '',
    iva: '',
    cantidad: 1  // Ahora se incluye cantidad
  });
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        console.log("API filters:", buildApiFilters());
        ...
        console.log("Datos cargados correctamente, estado del filtro:", filters);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSubmit = {
        ...
        cantidad: parseInt(formData.cantidad || 1) // Ahora se envía la cantidad
      };
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <Select 
                    ...
                    className="w-full"
                  >
                    <SelectTrigger id="filter-cliente" className="w-full">
                ...
                <div>
                  ...
                  <Select 
                    ...
                    className="w-full"
                  >
                    <SelectTrigger id="filter-empresa" className="w-full">
                    ...
                    {ventas.length === 0 ? (
                      ...
                      ventas.map((venta) => (
                        <TableRow key={venta.id}>
                          ...
                          <TableCell className="text-right">{formatCurrency(venta.valor || 0)}</TableCell>
                          <TableCell className="text-right">{formatCurrency(venta.iva || 0)}</TableCell>
                          <TableCell className="text-right font-bold">
                            {formatCurrency(parseFloat(venta.valor || 0) + parseFloat(venta.iva || 0))}
                          </TableCell>
                          <TableCell>
              ...
              <div className="p-4 bg-gray-50 rounded-lg border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{currentItem.cliente?.nombre || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Empresa:</p>
                    <p className="font-medium">{currentItem.empresa?.nombre || 'N/A'}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  <p>
                    ...
                    <span className="font-mono bg-white px-1.5 py-0.5 rounded border">
                      {currentItem.producto?.code || currentItem.producto?.codigo || 'N/A'}
                    </span>
                  </p>
                  <p>
                    <span className="text-gray-600">Nombre:</span>{" "}
                    <span className="font-medium">{currentItem.producto?.nombre || 'N/A'}</span>
                  </p>
                  <p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <p className="text-lg font-bold text-green-700">
                      {formatCurrency(parseFloat(currentItem.valor) + parseFloat(currentItem.iva))}
                    </p>
                  </div>
                </div>
              </div>
