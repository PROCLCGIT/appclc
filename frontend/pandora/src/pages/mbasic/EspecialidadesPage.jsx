...
const EspecialidadesPage = () => {
  ...
  const handleDelete = async (item) => {
    if (window.confirm('¿Está seguro de eliminar este registro?')) {
      try {
        await especialidadesService.delete(item.id);
        await loadData();
      } catch (err) {
        setError(err.message);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedItem) {
        await especialidadesService.update(selectedItem.id, formData);
      } else {
        await especialidadesService.create(formData);
      }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="bg-white shadow-sm rounded-md p-4 sm:p-6">
        <h1 className="text-2xl font-semibold mb-2">Especialidades</h1>
        <p className="text-gray-500 mb-4">Gestiona las especialidades del sistema</p>
      ...
      <FormDialog
        ...
        title={selectedItem ? 'Editar Especialidad' : 'Nueva Especialidad'}
